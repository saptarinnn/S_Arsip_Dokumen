<x-app-layout>
    ...
    <x-card title="{{ $title }}" :create="false">
        ...
        <x-form action="{{ route('role.update', $role->uuid) }}">
            @method('PUT')
            <x-form.input label="Nama Role" name="name" value="{{ $role->name }}" required autofocus />
            <div class="row">
                <div class="mb-4">
                            ...
                            <div class="col-3">
                                <div class="form-check mt-2">
                                    <input class="@error('permission') is-invalid @enderror form-check-input"
                                        ...
                                        id="{{ $data['uuid'] }}"
                                        {{ in_array($data['name'], $role_name) ? 'checked' : '' }} />
